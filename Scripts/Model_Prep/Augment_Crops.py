...
crops = r"C:\Users\sa553\Desktop\NPS\Crops_Recombined\Crops_All_Filtered"
# directory of augmented crops and polygon files
# outdirectory
augmentedCrops = r"C:\Users\sa553\Desktop\NPS\Crops_Recombined\color_training_set"
...
def transform_image(image_path, output_dir):
    ...
    transformations = {
        "_90": img.rotate(90, expand=True),
        "_180": img.rotate(180, expand=True),
        "_270": img.rotate(270, expand=True),
        "_mx": img.transpose(Image.FLIP_LEFT_RIGHT),
        "_my": img.transpose(Image.FLIP_TOP_BOTTOM)
    }
    
    for transform_name, transformed_img in transformations.items():
        output_path = os.path.join(output_dir, f"{base_filename}_{transform_name}.jpg")
...
for filename in os.listdir(crops):
    ...
    if filename.lower().endswith(('.jpg', '.jpeg')):
        ...
        if os.path.isfile(json_filepath):
            ...
            transform_image(os.path.join(crops, filename),augmentedCrops)
