...
for filename in os.listdir(crops):
    ...
    if filename.lower().endswith(('.jpg', '.jpeg')):
        ...
        shutil.copy(os.path.join(crops, filename), os.path.join(augmentedCrops, filename))

        # Load the associated JSON file, if present
        ...
        if os.path.isfile(json_filepath):
            ...
            # Copy the JSON file to the augmented directory
            shutil.copy(json_filepath, os.path.join(augmentedCrops, json_filename))

            # Rotate the image and save with the new filenames
            for angleO in [90, 180, 270]:  # Process 90, 180, and 270 degrees
                rotated_image = original_image.rotate(angleO)
                rotated_filename = os.path.splitext(filename)[0] + f"_{angleO}.jpg"
                rotated_image.save(os.path.join(augmentedCrops, rotated_filename), format='JPEG')

                # Rotate polygons in the JSON data
                angle = 270
                for shape in json_data["shapes"]:
                    rotated_polygon = rotate_polygon(shape["points"], -angle, center=(d/2, d/2))
                    shape["points"] = rotated_polygon

                # Update JSON data with rotated polygons and base64-encoded image data
                rotated_json_filename = os.path.splitext(rotated_filename)[0] + ".json"
                json_data["imagePath"] = rotated_filename
                json_data["imageData"] = image_to_base64(os.path.join(augmentedCrops, rotated_filename))
            ...
            mirrored_x_image.save(os.path.join(augmentedCrops, mirrored_x_filename), format='JPEG')

            # Mirror polygons over the x-axis
            for shape in json_data["shapes"]:
            ...
            mirrored_x_json_filename = os.path.splitext(mirrored_x_filename)[0] + ".json"
            json_data["imagePath"] = mirrored_x_filename
            json_data["imageData"] = image_to_base64(os.path.join(augmentedCrops, mirrored_x_filename))
            ...
            mirrored_y_image.save(os.path.join(augmentedCrops, mirrored_y_filename), format='JPEG')

            # Mirror polygons over the y-axis
            for shape in json_data["shapes"]:
                mirrored_y_polygon = mirror_polygon_y(shape["points"], d)
                shape["points"] = mirrored_y_polygon

            # Update JSON data with mirrored polygons and base64-encoded image data
            mirrored_y_json_filename = os.path.splitext(mirrored_y_filename)[0] + ".json"
            json_data["imagePath"] = mirrored_y_filename
            json_data["imageData"] = image_to_base64(os.path.join(augmentedCrops, mirrored_y_filename))
            ...
            print(f'No JSON file found for {filename}. Only the image has been saved.')
